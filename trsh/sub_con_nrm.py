...
#
def sub_con(n, m, x_k, x_d, px_l, px_u, g, dg, params):
#
...
#
    for i in range(n):
        x_l[i] = max(mov*x_k[i],px_l[i])
        x_u[i] = min(x_k[i]/mov,px_u[i])
#
...
#
    return [x,x_d,x_l,x_u]
#
# CONLIN: x in terms of dual variables 
#
def x_dual(x_d, n, m, x_k, g, dg, x_l, x_u):
#
...
#
    for j in range(n):
        x[j] = min(max(np.sqrt(tmpn[j]/tmpp[j])*x_k[j],x_l[j]),x_u[j])
#
    return x
#
# CONLIN: Dual function value
#
def con_dual(x_d, n, m, x_k, g, dg, x_l, x_u):
#
    x=x_dual(x_d, n, m, x_k, g, dg, x_l, x_u)
#
    W = g[0]
    for j in range(n):
        if dg[0][j] > 0e0:
            W = W + dg[0][j]*(x[j]/x_k[j]-1e0)*x_k[j]
        else:
            W = W - dg[0][j]*(x_k[j]/x[j])*(x_k[j]-x[j])
        for i in range(m):
            W = W + x_d[i]*g[i+1]
            if dg[i+1][j] > 0e0:
                W = W + dg[i+1][j]*(x[j]/x_k[j]-1e0)*x_k[j]*x_d[i]
            else:
                W = W - dg[i+1][j]*(x_k[j]/x[j])*(x_k[j]-x[j])*x_d[i]
#
    return -W
#
# CONLIN: Dual gradient
#
def dcon_dual(x_d, n, m, x_k, g, dg, x_l, x_u):
#
    x=x_dual(x_d, n, m, x_k, g, dg, x_l, x_u)
#
...
#
    for i in range(m):
        ...
        for j in range(n):
            if dg[i+1][j] > 0e0:
                dW[i] = dW[i] + dg[i+1][j]*(x[j]/x_k[j]-1e0)*x_k[j]
            else:
                dW[i] = dW[i] - dg[i+1][j]*(x_k[j]/x[j])*(x_k[j]-x[j])
#
    return -dW
#
